...
class SettingController extends AdminbaseController{
	...
	{
		$option=$this->options_model->where("option_name='tx_options'")->find();
		$info =json_decode($option['option_value'],true);
		$this->assign('tnumber',$info['tnumber']);
		$this->display();
	}
	public function tixing_post()
	{
		if(IS_POST)
		{
			$options=I('post.options/a');
			if(intval($options['tnumber']) == 0)
			{
				$this->ajaxReturn(array('status'=>1,'msg'=>'请输入大于0的整数'));
			}else
			{
				$data['option_name'] ='tx_options';
				$data['option_value'] = json_encode($options);
				if($this->options_model->where("option_name='tx_options'")->find()){
				$result=$this->options_model->where("option_name='tx_options'")->save($data);
				}else{
					$result=$this->options_model->add($data);
				}
	...
	// 复查提醒
    public function tixinglog()
    {
        ignore_user_abort();//关掉浏览器，PHP脚本也可以继续执行.
        set_time_limit(0);// 通过set_time_limit(0)可以让程序无限制的执行下去
        $interval = 1800;// 每隔半小时运行
        do{
          $run = include 'data/conf/config.php';
          if(!$run['isopen']) die('提醒结束');           
          //ToDo
          sleep($interval);// 等待5分钟
			file_put_contents('a0001log3.txt',date('Y-m-d H:i:s')."\n",FILE_APPEND);
          $hours = date('H');
          if($run['atime'] == $hours)
          {
            Tomembertixing();
