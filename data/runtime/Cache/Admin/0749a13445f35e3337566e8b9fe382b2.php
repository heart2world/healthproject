...
<body>
	<div class="wrap js-check-wrap">
		...
		<form class="well form-search" method="post" action="<?php echo U('Member/index');?>">	
			...
			距离复查时间（天）：
			<input type="text" name="number" style="width: 100px;" value="<?php echo ((isset($formget["number"]) && ($formget["number"] !== ""))?($formget["number"]):''); ?>" placeholder="输入天数">&nbsp;
		...
		<form class="js-ajax-form" action="" method="post">
			<table class="table table-hover table-bordered table-list">
				...
				<?php if(is_array($member)): foreach($member as $key=>$vo): ?><tr>
					...
					<td style="text-align: center;"><?php if($vo['againtime'] != ''): echo ($vo["againtime"]); else: ?>无<?php endif; ?></td>
					<td style="text-align: center;">
						...
						<?php else: ?>
							<a href="<?php echo U('Member/cancelban',array('id'=>$vo['id']));?>" class="btn btn-primary js-ajax-dialog-btn" style="padding: 2px 15px;color: white;background-color: #ccbd1d;"  data-msg="确认解冻该客户吗？">解冻</a><?php endif; ?>
						<a href="<?php echo U('Member/mdetail',array('id'=>$vo['id']));?>" class="btn btn-primary" style="padding: 2px 15px;color: white;background-color: #1dccaa;">个人信息</a>
						<a href="<?php echo U('Member/healthlog',array('id'=>$vo['id']));?>" class="btn btn-primary" style="padding: 2px 15px;color: white;background-color: #1dccaa;">健康日志</a>
						<a href="<?php echo U('Member/checklog',array('id'=>$vo['id']));?>" class="btn btn-primary" style="padding: 2px 15px;color: white;background-color: #1dccaa;">检查记录</a>
						<a href="<?php echo U('Member/report',array('id'=>$vo['id']));?>" class="btn btn-primary" style="padding: 2px 15px;color: white;background-color: #1dccaa;">录入报告</a>						
					</td>
